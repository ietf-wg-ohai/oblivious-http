...
chooses the DHKEM(X25519, HKDF-SHA256) and generates an X25519 key pair
...
The client then reads the oblivious request resource key configuration, selects a pair of KDF and AEAD and generates an X25519 key pair:
...
key from the request and a randomly selected value. This produces a salt of:
...
The AEAD Seal function is then used to encrypted the response, which is added
...
construct the AEAD key and nonce and extract the response.
